...
* The velocity of light is constant and universal: [it's $c$ in every frame.]{.fragment}
...
* Put a flashlight at $O''$.
* The flashlight emits a photon every $\Delta t''$ in its own frame (it's connected to a light clock with period $\Delta t''$).

## Part 1

* With what period do photons arrive at $O'$, according to $F'$?
	* If the flashlight were stationary, the period would just be $\Delta t''$.
	* But it's moving, so

	   1. It is time dilated — to $F'$, it emits a photon every $\gamma_u \Delta t''$.
      1. Each photon is emitted a distance $u'\gamma_u \Delta t''$ from the previous one.

## Part 1

* Each photon has to travel for an additional time $\frac{u'\gamma_{u'} \Delta t''}{c}=\gamma_{u'}\beta_{u'} \Delta t''$ than the previous to arrive at $O'$ (or less time if $u'<0$).
* Each photon arrives at $O'$ a time $\Delta t_\textrm{arr}'=\gamma_{u'}\Delta t''+ \gamma_{u'}\beta_{u'} \Delta t''=\gamma_{u'}\Delta t''(1+\beta_{u'})$ after the previous.

## Part 1

* The _period shift_ — the received period divided by the emitted period — is:

::: {.fragment}
$$
\begin{align*}
D(\beta_{u'})=\frac{\Delta t_\textrm{arr}'}{\Delta t''}&\class{fragment}{{}=\gamma_{u'}(1+\beta_{u'})}\\
&\class{fragment}{{}=\frac{(1+\beta_{u'})}{\sqrt{1-\beta_{u'}^2}}}
\end{align*}
$$
:::
...
$$
\begin{align*}
D(\beta_{u'})&=\frac{1+\beta_{u'}}{\sqrt{1-\beta_{u'}^2}}\\
&\class{fragment}{{}=\frac{(1+\beta_{u'})}{\sqrt{(1-\beta_{u'})(1+\beta_{u'})}}}\\
&\class{fragment}{{}=\frac{\sqrt{1+\beta_{u'}}}{\sqrt{1-\beta_{u'}}}}\\
&\class{fragment}{{}=\sqrt{\frac{1+\beta_{u'}}{1-\beta_{u'}}}}
\end{align*}
$$
...
* But the flashlight is also moving in $F$, and we know the period shift of moving flashlights based on their velocity...
	* It's $D(\beta)=\sqrt{\frac{1+\beta}{1-\beta}}$.
...
:::: {.column .fragment}
![Velocity Addition (source: me)](/images/5/velocity_addition.svg)
::::
:::
...
* Actually: $u=\frac{u'+v}{1+\frac{u'v}{c^2}}=\frac{1.8c}{1+0.81}=0.994c<c$ — good!
